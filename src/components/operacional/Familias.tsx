...
export function Familias({ tarefas, mes, snapshots = [], onFamilia }: { tarefas: TarefaOperacional[]; mes: string; snapshots?: SnapshotDia[]; onFamilia: (nome: string) => void }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      ...
      {registrados >= 2 && <Bloco titulo="Atrasadas por dia" acao={<span className="text-xs text-muted-foreground">{registrados} dias registrados</span>}>
        <ResponsiveContainer width="100%" height={250}><LineChart data={serieAtrasadas}><CartesianGrid vertical={false} stroke={CORES.borda} /><XAxis dataKey="dia" tickFormatter={(v) => v.slice(8, 10)} tick={{ fontSize: 10 }} /><YAxis allowDecimals={false} tick={{ fontSize: 10 }} /><Tooltip contentStyle={chartTooltipStyle} labelFormatter={(v) => `${v.slice(8, 10)}/${v.slice(5, 7)}`} /><Line type="monotone" dataKey="atrasadas" name="Atrasadas" stroke={CORES.atraso} strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer>
      </Bloco>}
