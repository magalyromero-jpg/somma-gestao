...
import { familias, imoveis, type Classificacao, type StatusLocacao } from "@/data/mock";
...
export default function Imoveis() {
  ...
  const cidades = Array.from(new Set(imoveis.map((i) => i.cidade))).sort();
  ...
  const filtered = useMemo(
    ...
    [familia, status, classif, cidade, q],
  );
  ...
  return (
    <>
      <PageHeader
        title="Imóveis"
        subtitle={`${filtered.length} de ${imoveis.length} no portfólio`}
        actions={
