...
import { familias, imoveis, kpisGlobais, kpisFamilia } from "@/data/mock";
...
export default function Dashboard() {
  ...
  const topFamilias = familias
    .map((f) => ({ ...f, ...kpisFamilia(f.id) }))
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <KpiCard
          ...
          value={formatBRL(k.valor_mercado, { compact: true })}
          icon={<TrendingUp className="h-4 w-4" />}
          delta={32.4}
        />
        <KpiCard
          label="Receita mensal"
          value={formatBRL(k.receita_mensal, { compact: true })}
          icon={<Wallet className="h-4 w-4" />}
          hint="aluguéis ativos"
        />
      </div>
        ...
        <Card className="lg:col-span-2 shadow-card">
          ...
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topFamilias} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
