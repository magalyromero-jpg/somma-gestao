  ...
  try {
    ...
    if (action === "listar_familias_bitrix") {
      ...
      const tarefas = allTasks
        ...
        .map((t: any) => ({ id: t.id, titulo: t.title, responsavel_id: t.responsibleId }));
    ...
    if (action === "tarefas_por_familia") {
      if (!familia_id || !bitrix_task_id) {
        return new Response(JSON.stringify({ error: "familia_id e bitrix_task_id são obrigatórios" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      if (!forceRefresh) {
      ...
      const tarefas = allTasks.map((t: any) => ({
        familia_id, bitrix_task_id: parseInt(t.id), titulo: t.title,
        descricao: t.description ?? null, status: mapStatus(t.status),
        ...
        prazo: t.deadline ?? null, marcadores: t.tags ?? [],
        link_bitrix: `https://sommainvestimentos.bitrix24.com.br/company/personal/user/1884/tasks/task/view/${t.id}/`,
        synced_at: new Date().toISOString(),
      }));
      if (tarefas.length > 0) {
        await supabase.from("bitrix_tarefas_cache").upsert(tarefas, { onConflict: "familia_id,bitrix_task_id" });
      } else {
      ...
      for (let i = 0; i < todasFamilias.length; i += LOTE) {
        ...
        const resultados = await Promise.all(lote.map(async (familia: any) => {
          const resAbertas = await fetch(`${BITRIX_URL}/tasks.task.list.json`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              filter: { "PARENT_ID": familia.id, "!STATUS": "5" },
          ...
          const hoje = abertas.filter((t: any) => {
            if (!t.deadline) return false;
            return new Date(t.deadline).toDateString() === agora.toDateString();
          }).length;
          return {
            id: familia.id, titulo: familia.title,
            responsavel_nome: familia.responsible?.name ?? null,
            ultima_atividade: familia.activityDate,
            total_abertas, atrasadas, alta_prioridade, hoje,
          };
        }));
        familias_com_resumo.push(...resultados);
      }

      familias_com_resumo.sort((a, b) => {
        if (b.atrasadas !== a.atrasadas) return b.atrasadas - a.atrasadas;
        return b.total_abertas - a.total_abertas;
      });

      const totais = {
        total_familias: familias_com_resumo.length,
        total_abertas: familias_com_resumo.reduce((s, f) => s + f.total_abertas, 0),
        total_atrasadas: familias_com_resumo.reduce((s, f) => s + f.atrasadas, 0),
        total_alta_prioridade: familias_com_resumo.reduce((s, f) => s + f.alta_prioridade, 0),
        total_hoje: familias_com_resumo.reduce((s, f) => s + f.hoje, 0),
      };
